...
class TestProductModel(unittest.TestCase):
    ...
    def test_deserialize_missing_field(self):
        """It should raise a DataValidationError if a required field is missing"""
    ...
    def test_update_without_id(self):
        """It should raise DataValidationError when updating a product without an ID"""
        product = ProductFactory()
    ...
    def test_deserialize_invalid_category(self):
        """It should raise a DataValidationError if the category is invalid"""
        product = ProductFactory()
        invalid_data = {
            "name": product.name,
            "description": product.description,
            "price": str(product.price),
            "available": product.available,
            "category": "INVALID_CATEGORY",  # Not a valid enum value
        }
        with self.assertRaises(DataValidationError) as context:
            product.deserialize(invalid_data)
    ...
    def test_deserialize_missing_data(self):
        """It should raise a DataValidationError if the input is None"""
        product = ProductFactory()
        invalid_data = None
        with self.assertRaises(DataValidationError) as context:
            product.deserialize(invalid_data)
    ...
    def test_find_by_price_string(self):
        """It should return products if the price is passed as a string"""
        product = ProductFactory()
        product.create()

        price = str(product.price)
        found_products = Product.find_by_price(price).all()
